...
class RobotArmControl:
    ...
    def setup_position_combobox(self):
        ...
        spacer = Frame(position_frame, height=10)  # 10은 간격의 높이입니다.
        spacer.pack()

        # 수신된 데이터를 표시할 라벨 추가
        self.received_label = Label(position_frame, text="", bg="black", fg="white")
    ...
    def start_serial_thread(self):
        if self.serial_thread is None or not self.serial_thread.is_alive():
            ...
            self.serial_thread = Thread(target=self.receive_serial_data)
    ...
    def update_received_label(self, text):
        self.received_label.config(text=text)
